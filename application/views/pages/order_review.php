                                ...
                                <div class="col-12 col-md-5">
                                    <div class="d-flex align-items-center">
                                        <a href="<?php echo base_url().'product/index/'.$product['pid'];?>">
                                            <?php 
                                                // Check if pimage contains full path or just filename
                                                $imagePath = $product['pimage'];
                                                if (strpos($imagePath, 'assets/') !== 0) {
                                                    // If it doesn't start with 'assets/', it might be a full path
                                                    if (file_exists($imagePath)) {
                                                        $imageUrl = base_url() . $imagePath;
                                                    } else {
                                                        // Try common product image locations
                                                        $imageUrl = base_url() . 'assets/images/products/' . basename($imagePath);
                                                        if (!file_exists('assets/images/products/' . basename($imagePath))) {
                                                            $imageUrl = base_url() . 'assets/img/no-image.png'; // fallback
                                                        }
                                                    }
                                                } else {
                                                    $imageUrl = base_url() . $imagePath;
                                            ...
                                            ?>
                                            <img src="<?php echo $imageUrl; ?>" class="cart-item-img" alt="<?php echo htmlspecialchars($product['pname']); ?>" onerror="this.src='<?php echo base_url('assets/img/no-image.png'); ?>'">
                                        </a>
                                        <div class="cart-title text-left">
                                            ...
                                            <?php 
                                                $weightLabel = '';
                                                if (isset($product['weight']) && isset($product['unit']) && $product['weight'] !== '') {
                                                    $weightLabel = 'Weight: ' . htmlspecialchars($product['weight']) . ' ' . htmlspecialchars($product['unit']);
                                                }
                                            ?>
                                            <span class="text-muted text-sm"><?php echo $weightLabel; ?></span>
                                            <br>
                                        </div>
                                    </div>
                                </div>
