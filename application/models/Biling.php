...
if(!defined('BASEPATH'))
    ...
    class Biling extends CI_Model{
        ...
        public function updateDeliveryDetails($userId, $deliveryDate, $deliveryTimeSlot, $deliveryInstructions){
            $data = [
        ...
        public function updateSpecialHandling($userId, $specialHandling){
            $data = [
                'handle_fragile' => $specialHandling['handle_fragile'],
                'keep_cold' => $specialHandling['keep_cold'],
                'contact_before' => $specialHandling['contact_before']
            ];
            $this->db->where('user_id', $userId);
            $this->db->update('billing_details', $data);   
        }
